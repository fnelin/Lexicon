...
const guitar: { brand: string; strings: number; isNew: boolean; description: () => void } = {
  brand: "Fender",
  strings: 6,
  isNew: true,
  description() {
...
const anyAsText: any = `Text i any sträng`;

console.log(`Övning 4`);
console.log(
  `Numret som var tillsammans med texten ${parseInt(numbersWithText)}\nNumret som var en textsträng ${Number(
    numbersAsText
  )}\nLängden på textsträngen är ${(anyAsText as string).length} tecken`
);
